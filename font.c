...
void lcd_put_ascii(int x, int y , unsigned int bcolor,unsigned int wcolor,unsigned char c)
{
	...
	for(i=0 ; i<16; i++)
	{
		...
		{
			if(byte & (1<<j))
				show_put_pixel(x+(7-j),y+i,wcolor);
			else
			{
				show_put_pixel(x+(7-j),y+i,bcolor);  //显示字符时的底色
			}
		}
	}
}
...
//只能显示很小的字串
void show_charword(int x,int y,int d, unsigned int wcolor,unsigned int bcolor,char *str)//d表示字符间隔,flag 表示中英文，0表示英文
{
	char *p;
	int i ;
	...
	{
		lcd_put_ascii(x+(8+d)*i,y,bcolor,wcolor,*p);
	}
	
}

void lcd_put_gbk(int x, int y, unsigned int wcolor,unsigned int bcolor,unsigned char *str )
{
	int i, j,k ;
	unsigned char byte;
	// 根据汉字库找到 汉字 起始地址
	...
	for(i=0; i< 16 ;i++)
	{
		for(j=0 ; j<2 ;j++)
		{
			...
			for(k=7 ; k>=0; k--)
			{
				if(byte & (1<<k))
				{
					show_put_pixel(x+(7-k)+j*8,y+i,wcolor);
				}
				else
				{
					show_put_pixel(x+(7-k)+j*8,y+i,bcolor);				
				}
			}
		}	
	}
